...
$getVideo = '<div class="border-body" style="border: 1px solid grey; " >
		...
		foreach($this->get('streams', []) as $format) {
			if ($format['size'] != 0){
				...
				if ($format['show_direct_url'] === true) {
					$getVideo .= ' <a class="btn btn-primary btn-download" href="'.$format['direct_url'].'" class="mime"><i class="glyphicon glyphicon-download-alt"></i> Direct</a>';
				}
				if ($format['show_proxy_url'] === true) {
					$getVideo .= ' <a class="btn btn-default btn-download" href="'.$format['proxy_url'].'" class="mime"><i class="glyphicon glyphicon-download-alt"></i> Proxy</a>';
				}
				...
				$getVideo .= '<a class="btn btn-block" href="#" onclick="location.reload();"><i class="glyphicon glyphicon-refresh"></i> Nothing</a>';
		}
		$getVideo .= '</ul>
		<hr />
